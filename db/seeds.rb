...
def hipster_paragraphs(min, max)
  max_rand = max - min
  FFaker::HipsterIpsum.paragraphs(rand(max_rand) + min).join("\n")
end
...
def random_email
  "#{FFaker::Internet.safe_email}#{Time.new.to_i}"
end
...
hash_posts = 20.times.map do
  {
    title: FFaker::HipsterIpsum.paragraph[0...200],
